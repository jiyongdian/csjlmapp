...
/** POST /api/admin/novels/batch-delete — 批量删除（管理员） */
  ...
  try {
    ...
    for (const id of ids) {
      try {
        ...
        if (novel) await novelManager.delete(id, novel.userId);
        deleted++;
      } catch (e) {
        console.error('[Admin Novels] 删除失败 id=' + id + ':', e);
        failed.push(id);
      }
    }
    ...
    console.error('[Admin Novels] 批量删除失败:', error);
