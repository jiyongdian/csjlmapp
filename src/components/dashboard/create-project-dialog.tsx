...
export function CreateProjectDialog({
  ...
  const [artStyleCategory, setArtStyleCategory] = useState<string>("all");
                  ...
                  {presetsLoading ? (
                    ...
                    <div className="space-y-3">
                      ...
                      {presets.length > 0 && (
                        <div className="flex gap-1.5 flex-wrap">
                          {[
                            ...
                            { key: "2D", label: "2D动画" },
                            { key: "3D", label: "3D渲染" },
                            { key: "realpeople", label: "真人实拍" },
                          ].map((cat) => (
                            <button
                              key={cat.key}
                              type="button"
                              onClick={() => setArtStyleCategory(cat.key)}
                              className={cn(
                                "px-2.5 py-1 rounded-lg text-xs font-medium transition-all",
                                artStyleCategory === cat.key
                      ...
                      <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
                        {presets
                          .filter((p) => artStyleCategory === "all" || p.category === artStyleCategory)
                          .map((preset) => (
                          <button
                            key={preset.key}
                            type="button"
