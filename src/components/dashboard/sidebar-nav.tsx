...
const workspaceItems: SidebarItem[] = [
  { key: "open-book", label: "一键开书", icon: BookOpen, href: "/workspace?tab=open-book", iconColor: "text-violet-400" },
  { key: "short-story", label: "短篇写作", icon: PenTool, href: "/workspace?tab=short-story", iconColor: "text-violet-400" },
  { key: "deconstruct", label: "小说拆文", icon: Search, href: "/workspace?tab=deconstruct", iconColor: "text-violet-400" },
  { key: "review", label: "质量审查", icon: Star, href: "/workspace?tab=review", iconColor: "text-violet-400" },
  { key: "deslop", label: "去AI味", icon: Palette, href: "/workspace?tab=deslop", iconColor: "text-violet-400" },
];
...
export function SidebarNav({ onNavigate, project: projectProp }: { onNavigate?: () => void; project?: Project | null }) {
  ...
  const getIsActive = (href: string) => {
    ...
    // For workspace items with query params, match by pathname + tab param
    if (href.startsWith('/workspace')) {
      const tabMatch = href.match(/tab=([^&]+)/);
      if (tabMatch) {
        const currentTab = pathname.match(/tab=([^&]+)/);
        return pathname.startsWith('/workspace') && currentTab?.[1] === tabMatch[1];
      }
