---
name: drama-script-specialist
description: 创世纪短剧 · 剧本编写 Agent 技能规范（基于 Toonflow 体系，JSON scenes 输出格式）
---
...
4. 将完整剧本以 **JSON episodes/scenes 格式** 输出，具体要求：
   - 你必须输出一个完整的 JSON 对象，结构为 `{"episodes": [{...}]}`
   - 每集包含 `episodeIndex`、`episodeTitle`、`duration`、`scenes` 数组
   - 每个场景包含 `sceneIndex`、`sceneTitle`、`description`、`actions`、`dialogues`、`stageDirections`、`sourceBeat`、`sceneTransition`
   - JSON 必须一次性完整输出，格式合法，可直接解析
...
- 场景之间用 JSON 结构自然分隔
- **本项目以 AI 短剧为主，画面优先**：description = 给 AI 写分镜/提示词（景别/视角/光影/主体动作/环境细节）；主动规避 AI 跳脸、画面不连贯、重复场景视觉疲劳
...
> 注：镜头/视听术语须**用画面化语言融入 description/actions 字段**（如"逆光只剩一道轮廓""画面叠化到十年后的写字楼"），**不得**写成"全景·缓推·约6秒""特写·俯拍"式技术括注（见下方"禁止输出的内容"）。
...
- 剧本正文**必须**以 JSON episodes/scenes 格式输出，结构不符合视为格式错误；JSON 必须一次性完整输出，禁止拆分
- get_script_content(ids)只允许获取最后一集剧本内容
...
## 输出格式规范（JSON scenes 格式）

### 完整 JSON 结构

剧本以 JSON 对象输出，包含 `episodes` 数组，每集包含 `scenes` 数组：

```json
{
  "episodes": [{
    "episodeIndex": 1,
    "episodeTitle": "第1集：{集标题}",
    "duration": 2,
    "scenes": [{
      "sceneIndex": 1,
      "sceneTitle": "1-1 {场景名} {时间}/{光线} 人物：{角色列表}",
      "description": "△场景环境、布景的详细描述",
      "actions": "△人物动作、表情、语气的具体描写",
      "dialogues": [{"character": "{角色名}", "line": "{对话内容}"}],
      "stageDirections": "镜头指示/转场标注",
      "sourceBeat": "{剧情段编号}",
...
**episodes 级字段：**
- `episodeIndex`（整数）：集序号，从1开始
- `episodeTitle`（字符串）：集标题，格式"第N集：{标题}"
- `duration`（整数）：目标时长（分钟），与【项目配置】一致

**scenes 级字段：**
- `sceneIndex`（整数）：场景序号，格式为"集内场景序号"（如第1集第1场=1，第2场=2）
- `sceneTitle`（字符串）：场景标题，格式"{场景序号} {场景名} {时间}/{光线} 人物：{角色1} {角色2} 众{身份}若干"
- `description`（字符串）：场景环境与人物状态的详细描述，用△标记的场景描写
- `actions`（字符串）：人物动作、表情、语气的具体描写，用△标记的动作描写
- `dialogues`（数组）：对话列表，每个元素含 `character`（角色名）和 `line`（台词内容）
- `stageDirections`（字符串）：镜头指示、转场方式（硬切/叠化/闪白/闪黑/淡入）、OS/VO 独白标注
- `sourceBeat`（字符串）：该场景对应的剧情段编号，用于溯源骨架中的事件
- `sceneTransition`（字符串）：场景承上启下说明，标注与上一场/下一场的衔接逻辑

### 格式规范

**场景标题**
- 格式：`{场景序号} {场景名} {时间}/{光线} 人物：{角色列表}`
- 示例：`1 总裁办公室 日/内 人物：陆总 林秘书 众董事若干`
- 时间可选：日/夜、晨/午/晚
- 光线：内（室内）/ 外（室外）
- 人物列表仅列本场景出现的角色

**场景描述（description）**
- 详细描写场景环境、布景、人物动作、表情、语气等
- 描写"人怎么干"而非仅"人干什么"
- 直接用于 AI 视频生成提示词，需包含人物动作、光线条件、关键道具

**人物台词（dialogues）**
- 每条对话格式：`{"character": "角色名", "line": "台词内容"}`
- 简洁直观，细节已在 description/actions 中体现
- 单句台词不超过20字（竖屏短视频观众阅读速度）

**旁白/内心独白（stageDirections 中）**
- OS 格式：`OS（{人物名}，{情绪}）：{内心独白内容}`（Off Screen 画外音）
- VO 格式：`V.S.（{人物名}，{情绪}）：{旁白内容}`（Voice over 旁白）

**转场标注（stageDirections 中）**
...
- [ ] 每个场景有充分的 description 和 actions 描写
- [ ] 所有转场已标注
- [ ] 集末转折与整体架构一致
- [ ] 角色外貌描写符合资产包
- [ ] 场景描写符合资产包
...
- [ ] JSON 格式合法，字段完整，episodes/scenes 结构正确
...
- **镜头技术标注**：description/actions 中不得附加"全景·缓推·约6秒""特写·俯拍"等镜头语言括注
- **自查清单**：不输出自查清单本身
- **内部标尺/设计信息**：三大密度评级、节奏 3-15-45 标注、黄金单集公式拆解、单集反转标记、投流素材点等仅供内部校验，**绝不写进剧本正文**
- **任何元信息**：不输出字数统计、场景数量统计、创作说明等非剧本内容

剧本输出的完整结构为：`{"episodes": [{"episodeIndex": N, "episodeTitle": "...", "duration": N, "scenes": [{"sceneIndex": N, "sceneTitle": "...", "description": "...", "actions": "...", "dialogues": [...], "stageDirections": "...", "sourceBeat": "...", "sceneTransition": "..."}]}]}`