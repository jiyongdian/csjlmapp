---
name: ai-dehumanizer
description: 创世纪去AI味改写 — 融合 humanize-text-main 四套方法论 + Standard 管线 + stop-slop/humanizer-zh/avoid-ai-writing（简体中文）
---

# 创世纪去 AI 味改写（De-AI Humanizer）
...
**创世纪适配（简体中文输入）**：不依赖外部翻译 API 时，用 **「语义保真 + 结构扰动式改写」** 模拟翻译链效果——大幅调整句序、主语位置、修饰语前后置，如同经历了一轮「远距离回译」。
...
**创世纪适配**：若用户提供 **AI 检测参考**（概率、维度分、修改建议），则：
1. 全文先完成 Method 2 改写；
...
**创世纪适配**：在纯 LLM 场景下，用 **「换表述引擎」思维**——同一意思交替使用：短句直述 / 长句铺陈 / 对话拆开 / 动作先行，避免全篇同一「改写腔」。
...
## oh-story 增强：网文去 AI 味专项

处理网文正文时，按「改味优先、少改保剧情」执行：

- **优先保留**：人名、地名、境界、道具、数值、伏笔、章末钩子、角色口癖。
- **重点清理**：过度圆滑、句式过齐、解释过满、情绪标签直说、连续排比、每段都像总结。
- **替换策略**：把抽象情绪换成具体动作，把宏大判断换成现场反应，把工整连接词换成自然停顿。
- **删除上限**：轻度只微调，中度可合并重复句，重度才重写局部；不得删掉剧情功能段。
- **网文禁用倾向**：慎用「值得注意的是」「不禁」「仿佛一切都」「命运的齿轮」「空气中弥漫着」「某种难以言喻」「内心深处」等模板表达。
- **复核标准**：改完仍要满足前后因果、角色状态、伏笔与 15 项一致性，不能为了去 AI 味改丢信息。
